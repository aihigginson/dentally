...
RETURN COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""acceptance_rate"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Target Value]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""acceptance_rate"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value])) / 100",
    "#,##0.0%");
...
add("Open Courses Target",
    @"VAR sel_site = SELECTEDVALUE('List Practice Sites'[pk_Practice_Site], -1)
RETURN COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Target Value]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value]))",
    "#,##0");
...
add("Open Courses Without Appointment Target",
    @"VAR sel_site = SELECTEDVALUE('List Practice Sites'[pk_Practice_Site], -1)
RETURN COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses_without_appt"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Target Value]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses_without_appt"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value]))",
    "#,##0");
...
add("Exam Ratio Target",
    @"VAR sel_site = SELECTEDVALUE('List Practice Sites'[pk_Practice_Site], -1)
RETURN COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""exam_ratio"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Target Value]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""exam_ratio"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value])) / 100",
    "#,##0.0%");
...
VAR target   = COALESCE(
    ...
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""acceptance_rate"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value])) / 100
VAR band     = COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""acceptance_rate"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Variance]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""acceptance_rate"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Variance]))
VAR diff_pp  = (actual - target) * 100
RETURN SWITCH(TRUE(),
...
VAR target   = COALESCE(
    ...
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value]))
VAR band     = COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Variance]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Variance]))
VAR pct      = DIVIDE(actual - target, ABS(target)) * 100
RETURN SWITCH(TRUE(),
...
VAR target   = COALESCE(
    ...
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses_without_appt"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value]))
VAR band     = COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses_without_appt"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Variance]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses_without_appt"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Variance]))
VAR pct      = DIVIDE(actual - target, ABS(target)) * 100
RETURN SWITCH(TRUE(),
    ISBLANK(target),   ""#FFFFFF"",
    pct <= -band,  ""#1a7f3c"",
...
VAR target   = COALESCE(
    ...
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""exam_ratio"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value])) / 100
VAR band     = COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""exam_ratio"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Variance]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""exam_ratio"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Variance]))
...
add("Open Courses Value Target",
    @"VAR sel_site = SELECTEDVALUE('List Practice Sites'[pk_Practice_Site], -1)
RETURN COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses_value"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Target Value]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses_value"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value]))",
    "£#,##0");
...
add("Average Plan Value Target",
    @"VAR sel_site = SELECTEDVALUE('List Practice Sites'[pk_Practice_Site], -1)
RETURN COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""avg_plan_value"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Target Value]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""avg_plan_value"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value]))",
    "£#,##0");
...
VAR target   = COALESCE(
    ...
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses_value"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value]))
VAR band     = COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses_value"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Variance]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""open_courses_value"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Variance]))
VAR pct    = DIVIDE(actual - target, ABS(target)) * 100
RETURN SWITCH(TRUE(),
    ISBLANK(actual), ""#E0E0E0"",
    ISBLANK(target), ""#FFFFFF"",
...
VAR target   = COALESCE(
    ...
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""avg_plan_value"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Target Value]))
VAR band     = COALESCE(
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""avg_plan_value"" && '_Targets'[fk_Practice_Site] = sel_site && sel_site <> -1), '_Targets'[Variance]),
    MAXX(FILTER('_Targets', '_Targets'[Metric] = ""avg_plan_value"" && '_Targets'[fk_Practice_Site] = -1), '_Targets'[Variance]))
VAR pct    = DIVIDE(actual - target, ABS(target)) * 100
RETURN SWITCH(TRUE(),
    ISBLANK(target), ""#FFFFFF"",
    pct >= band,     ""#1a7f3c"",

--DECLARE @i BIGINT=0, @u BIGINT=0, @d BIGINT=0; EXEC [Bronze].[usp_Load_Users] @Tenant_ID=1, @Full_Refresh=1, @Run_Inserts=@i OUT, @Run_Updates=@u OUT, @Run_Deletes=@d OUT;
--------------------------------------------------------------------
...
--    *02     15/05/2026  AIH Load Created_At, Updated_At (pending Stage_Ingest refresh)
...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO Bronze.Users (Tenant_ID, ID, First_Name, Last_Name, Email, Role, DW_Loaded_At)
        SELECT src.Tenant_ID, src.ID, src.First_Name, src.Last_Name, src.Email, src.Role, SYSUTCDATETIME()
        FROM #src AS src
        ...
        IF @Full_Refresh = 1
        BEGIN
            DELETE tgt FROM Bronze.Users AS tgt

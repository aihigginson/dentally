...
--    *04     22/05/2026  AIH Add Tenant_Count (1 real, 0 sentinel) for SUM-based measures
...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO Gold.Dim_Tenants (pk_Tenant, Tenant_ID, Tenant_Name, Is_Active, Tenant_Count, DW_Created_At, DW_Updated_At)
        SELECT
            @pk_Tenant_base + ROW_NUMBER() OVER (ORDER BY src.Tenant_ID),
            src.Tenant_ID, src.Tenant_Name, src.Is_Active, src.Tenant_Count, SYSUTCDATETIME(), SYSUTCDATETIME()
        FROM #src src
        ...
        INSERT INTO Gold.Dim_Tenants (pk_Tenant, Tenant_ID, Tenant_Count, DW_Created_At, DW_Updated_At)
        SELECT -1, -1, 0, SYSUTCDATETIME(), SYSUTCDATETIME()

--DECLARE @i BIGINT=0, @u BIGINT=0, @d BIGINT=0; EXEC [Silver].[usp_Load_Invoice_Items] @Mode='PROD', @Run_Inserts=@i OUT, @Run_Updates=@u OUT, @Run_Deletes=@d OUT;
--------------------------------------------------------------------
--  Stored Procedure :  Silver.usp_Load_Invoice_Items
--  Author           :  AIH
...
BEGIN
    ...
    BEGIN TRY
        ...
        FROM #src AS src
        WHERE NOT EXISTS (
            SELECT 1 FROM [Silver].[Invoice_Items] AS tgt WHERE tgt.[Tenant_ID] = src.[Tenant_ID] AND tgt.[Id] = src.[Id]
        );

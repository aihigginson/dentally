...
-- -- Metric Actuals: materialised global value (all FYs) -- reconciles to the source each DAX measure sums --
,('ACTUALS_TOTAL_REVENUE_T11','Metric Actuals','Gold','currency','SELECT SUM(Actual_Value) FROM Gold.Fact_Metric_Actuals WHERE Tenant_ID = 11 AND Metric = ''total_revenue'' AND fk_Practice_Site = -1 AND fk_Practitioner = -1','Materialised total_revenue (global, all FYs) -- must equal GOLD_TOTAL_REVENUE_T11',1)
,('ACTUALS_NHS_REVENUE_T11','Metric Actuals','Gold','currency','SELECT SUM(Actual_Value) FROM Gold.Fact_Metric_Actuals WHERE Tenant_ID = 11 AND Metric = ''nhs_revenue'' AND fk_Practice_Site = -1 AND fk_Practitioner = -1','Materialised nhs_revenue (global, all FYs) -- must equal GOLD_NHS_REVENUE_INV_T11',1)
,('ACTUALS_PRIVATE_REVENUE_T11','Metric Actuals','Gold','currency','SELECT SUM(Actual_Value) FROM Gold.Fact_Metric_Actuals WHERE Tenant_ID = 11 AND Metric = ''private_revenue'' AND fk_Practice_Site = -1 AND fk_Practitioner = -1','Materialised private_revenue (global, all FYs) -- must equal GOLD_PRIVATE_REVENUE_INV_T11',1)
,('ACTUALS_NEW_PATIENTS_T11','Metric Actuals','Gold','count','SELECT SUM(Actual_Value) FROM Gold.Fact_Metric_Actuals WHERE Tenant_ID = 11 AND Metric = ''new_patients'' AND fk_Practice_Site = -1 AND fk_Practitioner = -1','Materialised new_patients (global, all FYs) -- must equal GOLD_AGG_NEW_PATIENTS_T11',1)
...
,('GOLD_FKNULL_INVITEM_DATE_DUE_T11','FK Null','Gold','count','SELECT COUNT(*) FROM Gold.Fact_Invoices WHERE Tenant_ID = 11 AND fk_Date_Due IS NULL','NULL count of fk_Date_Due in Gold.Fact_Invoices (tenant 11; moved to invoice grain)',1)
,('GOLD_FKNULL_INVITEM_DATE_PAID_T11','FK Null','Gold','count','SELECT COUNT(*) FROM Gold.Fact_Invoices WHERE Tenant_ID = 11 AND fk_Date_Paid IS NULL','NULL count of fk_Date_Paid in Gold.Fact_Invoices (tenant 11; moved to invoice grain)',1)
...
,('GOLD_PATIENTS_WITH_OUTSTANDING_T11','KPI Value','Gold','count','SELECT COUNT(DISTINCT fk_Patient) FROM Gold.Fact_Invoices WHERE Tenant_ID = 11 AND Is_Invoice_Outstanding = 1','Distinct patients with an outstanding invoice (invoice grain)',1)

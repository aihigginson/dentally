--DECLARE @i BIGINT=0, @u BIGINT=0, @d BIGINT=0; EXEC [Bronze].[usp_Load_Sites] @Tenant_ID=1, @Full_Refresh=1, @Run_Inserts=@i OUT, @Run_Updates=@u OUT, @Run_Deletes=@d OUT;
--------------------------------------------------------------------
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @Full_Refresh = 1
        BEGIN
            DELETE tgt FROM Bronze.Sites AS tgt
            WHERE tgt.Tenant_ID = @Tenant_ID
              AND NOT EXISTS (SELECT 1 FROM #src WHERE Site_ID = tgt.Site_ID);

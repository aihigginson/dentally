...
**6.2 Retention of patient data while a practice is active — PII held for active patients only.** Identifying/contact data (name, email, phone) is **retained only for active patients**, because the operational purpose (contacting patients) only applies to them. When a patient becomes **inactive**, their identifying/contact fields are **obfuscated** in the analytics layers (Silver → Gold → Power BI) while non-identifying operational/financial history is kept for trend analytics — implemented in release **V013**. Active-patient history is retained while the practice is active **[CONFIRM the period — full Dentally history vs e.g. last 7 years]**. A **DSAR / right-to-erasure workflow** remains **[DECISION NEEDED]** (note: erasure is simplified by the minimisation above and by inactive-patient obfuscation).

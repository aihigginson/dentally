...
2. **Drop special-category / PII** on patients (DPIA V011/V012 — these MUST NOT land; `date_of_birth` was removed from this list by V171 and now DOES land, see DPIA §7.3): `gender` (bool), `ethnicity`, `nhs_number`, `ni_number`, `pps_number`, `medical_alert`(bool)/`medical_alert_text`, `special_needs`, `occupation`, `school_name`, `emergency_contact_*`, `proof_of_identification`, `suspicious_identity`, `image_url`. KEEP: name, `date_of_birth` (V171), contact (phone/email + `use_email`/`use_sms`/`preferred_phone_number`), `marketing`, recall dates/intervals, `dentist_id`/`hygienist_id`, `acquisition_source_id`, `payment_plan_id`, `site_id`, `active`, `account_id`, town/postcode/county.
